...
export default function Transfers() {
  ...
  const { data: items = [] } = useQuery<any[]>({ queryKey: ["/api/items"], queryFn: () => customFetch("/api/items") });
  const { data: transfers = [], isLoading } = useQuery<any[]>({ queryKey: ["/api/transfers"], queryFn: () => customFetch("/api/transfers") });
  useEffect(() => { if (user?.role !== "admin" && user?.locationId) setFrom(String(user.locationId)); }, [user]);
  const mutate = useMutation({ mutationFn: async ({ url, method, body }: { url: string; method: string; body?: unknown }) => customFetch(url, { method, body: body ? JSON.stringify(body) : undefined, headers: { "Content-Type": "application/json" } }), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/transfers"] }); queryClient.invalidateQueries({ queryKey: ["/api/items"] }); }, onError: (error: any) => toast({ title: "Ошибка", description: error?.message || "Операция не выполнена", variant: "destructive" }) });
  function createTransfer() { if (!from || !to || !itemId || !quantity) return; mutate.mutate({ url: "/api/transfers", method: "POST", body: { fromLocationId: Number(from), toLocationId: Number(to), itemId: Number(itemId), quantity: Number(quantity), note } }); setFormOpen(false); setTo(""); setItemId(""); setQuantity(""); setNote(""); }
  ...
  return <div className="space-y-6">
    ...
    {formOpen && <Card><CardHeader><CardTitle>Новое перемещение</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
      ...
      <div className="space-y-2"><Label>Количество</Label><Input type="number" min="0.001" step="0.001" value={quantity} onChange={(e) => setQuantity(e.target.value)} /></div>
      ...
      <div className="flex gap-2 md:col-span-2"><Button onClick={createTransfer} disabled={mutate.isPending}>Отправить</Button><Button variant="outline" onClick={() => setFormOpen(false)}>Отмена</Button></div>
    </CardContent></Card>}
