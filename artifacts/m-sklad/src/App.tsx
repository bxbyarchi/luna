...
import KassaReports from "@/pages/kassa-reports";
import KassaCosting from "@/pages/kassa-costing";
...
function RoleCheck({ component: Component, minRole, domain = "warehouse" }: { component: React.ComponentType; minRole: AppRole; domain?: "warehouse" | "venue" }) { const { canDo, canDoVenue, isLoading } = useCurrentUser(); if (isLoading) return <div className="min-h-[100dvh] flex items-center justify-center"><div className="h-8 w-8 rounded-full border-2 border-primary border-t-transparent animate-spin" /></div>; const allowed = domain === "venue" ? canDoVenue(minRole) : canDo(minRole); if (!allowed) return <AppLayout><AccessDenied /></AppLayout>; return <AppLayout><Component /></AppLayout>; }
function RoleProtectedRoute({ component: Component, minRole, domain }: { component: React.ComponentType; minRole: AppRole; domain?: "warehouse" | "venue" }) { const { isLoaded, isSignedIn } = useAuth(); if (!isLoaded) return <div className="min-h-[100dvh] flex items-center justify-center"><div className="h-8 w-8 rounded-full border-2 border-primary border-t-transparent animate-spin" /></div>; if (!isSignedIn) return <Redirect to="/" />; return <RoleCheck component={Component} minRole={minRole} domain={domain} />; }
function OnboardingPage() { const { isLoaded, isSignedIn } = useAuth(); if (!isLoaded) return <div className="min-h-[100dvh] flex items-center justify-center bg-background"><div className="h-8 w-8 rounded-full border-2 border-primary border-t-transparent animate-spin" /></div>; if (!isSignedIn) return <Redirect to="/" />; return <Onboarding />; }
...
function ClerkProviderWithRoutes() { const [, setLocation] = useLocation(); return <ClerkProvider publishableKey={clerkPubKey} proxyUrl={clerkProxyUrl} appearance={clerkAppearance} signInUrl={`${basePath}/sign-in`} signUpUrl={`${basePath}/sign-up`} localization={{ ...ruRU, signIn: { ...ruRU.signIn, start: { ...ruRU.signIn?.start, title: "Войти в Северное сияние", subtitle: "Доступ к системе управления ярмаркой" } }, signUp: { ...ruRU.signUp, start: { ...ruRU.signUp?.start, title: "Создать аккаунт", subtitle: "Регистрация в системе «Северное сияние»" } } }} routerPush={(to) => setLocation(stripBase(to))} routerReplace={(to) => setLocation(stripBase(to), { replace: true })}><QueryClientProvider client={queryClient}><ClerkAuthTokenProvider /><Switch><Route path="/" component={HomeRedirect} /><Route path="/sign-in/*?" component={SignInPage} /><Route path="/sign-up/*?" component={SignUpPage} /><Route path="/dashboard"><ProtectedRoute component={Dashboard} /></Route><Route path="/items"><ProtectedRoute component={Items} /></Route><Route path="/transfers"><ProtectedRoute component={Transfers} /></Route><Route path="/houses"><RoleProtectedRoute component={Houses} minRole="location_admin" domain="venue" /></Route><Route path="/registers"><RoleProtectedRoute component={Registers} minRole="location_admin" domain="venue" /></Route><Route path="/shifts"><RoleProtectedRoute component={Shifts} minRole="cashier" domain="venue" /></Route><Route path="/kassa-dashboard"><RoleProtectedRoute component={KassaDashboard} minRole="location_admin" domain="venue" /></Route><Route path="/kassa-reports"><RoleProtectedRoute component={KassaReports} minRole="location_admin" domain="venue" /></Route><Route path="/kassa-costing"><RoleProtectedRoute component={KassaCosting} minRole="location_admin" domain="venue" /></Route><Route path="/categories"><RoleProtectedRoute component={Categories} minRole="manager" /></Route><Route path="/receipts"><ProtectedRoute component={Receipts} /></Route><Route path="/write-offs"><RoleProtectedRoute component={WriteOffs} minRole="accountant" /></Route><Route path="/inventory-audits"><ProtectedRoute component={InventoryAudits} /></Route><Route path="/rentals"><ProtectedRoute component={Rentals} /></Route><Route path="/staff"><RoleProtectedRoute component={Staff} minRole="manager" /></Route><Route path="/audit-log"><ProtectedRoute component={AuditLog} /></Route><Route path="/reports"><RoleProtectedRoute component={Reports} minRole="warehouse_chief" /></Route><Route path="/settings"><ProtectedRoute component={Settings} /></Route><Route path="/onboarding"><OnboardingPage /></Route><Route component={NotFound} /></Switch></QueryClientProvider></ClerkProvider>; }
