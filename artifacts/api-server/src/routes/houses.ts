...
router.post("/houses", requireAuth(), requireRole("admin"), async (req: Request, res: Response) => {
  const { name, locationId, isActive } = req.body;
  if (!name) { res.status(400).json({ error: "name required" }); return; }
  const locId = Number(locationId);
  if (!locId || !(await locationExists(locId))) { res.status(400).json({ error: "Некорректный склад/площадка" }); return; }
...
router.patch("/houses/:id", requireAuth(), requireRole("admin"), async (req: Request, res: Response) => {
...
router.delete("/houses/:id", requireAuth(), requireRole("admin"), async (req: Request, res: Response) => {
  const id = Number(req.params.id);
  const [row] = await db.delete(housesTable).where(eq(housesTable.id, id)).returning();
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
