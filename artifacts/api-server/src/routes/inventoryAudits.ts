...
router.get("/inventory-audits", requireAuth(), async (req: Request, res: Response) => {
  ...
  const rows = await db.select({ id: inventoryAuditsTable.id, title: inventoryAuditsTable.title, locationId: inventoryAuditsTable.locationId, locationName: locationsTable.name, status: inventoryAuditsTable.status, createdAt: inventoryAuditsTable.createdAt, submittedAt: inventoryAuditsTable.submittedAt }).from(inventoryAuditsTable).leftJoin(locationsTable, eq(inventoryAuditsTable.locationId, locationsTable.id)).where(conditions.length ? and(...conditions) : undefined).orderBy(sql`${inventoryAuditsTable.createdAt} DESC`);
  res.json(rows);
});
...
router.get("/inventory-audits/:id", requireAuth(), async (req: Request, res: Response) => {
  ...
  const auditItems = await db.select({ id: auditItemsTable.id, auditId: auditItemsTable.auditId, itemId: auditItemsTable.itemId, itemName: itemsTable.name, systemStock: auditItemsTable.systemStock, actualStock: auditItemsTable.actualStock, discrepancy: sql<string>`CASE WHEN ${auditItemsTable.actualStock} IS NOT NULL THEN CAST(${auditItemsTable.actualStock} AS DECIMAL) - CAST(${auditItemsTable.systemStock} AS DECIMAL) ELSE NULL END` }).from(auditItemsTable).leftJoin(itemsTable, eq(auditItemsTable.itemId, itemsTable.id)).where(eq(auditItemsTable.auditId, id));
...
router.patch("/inventory-audits/:id", requireAuth(), requireRole("admin"), async (req: Request, res: Response) => {
  ...
  for (const item of items) if (item.itemId !== undefined && item.actualStock !== undefined && Number(item.actualStock) >= 0) await db.update(auditItemsTable).set({ actualStock: String(item.actualStock) }).where(and(eq(auditItemsTable.auditId, id), eq(auditItemsTable.itemId, Number(item.itemId))));
  const audit = await db.query.inventoryAuditsTable.findFirst({ where: eq(inventoryAuditsTable.id, id) });
