...
import { db, transfersTable, warehouseStockTable, locationsTable, itemsTable, usersTable } from "@workspace/db";
...
router.get("/transfers", requireAuth(), async (req: Request, res: Response) => {
  ...
  const rows = await db.select({
    ...
    fromLocationName: sql<string>`${locationsTable.name}`,
    ...
    createdByClerkId: transfersTable.createdByClerkId,
    acceptedByClerkId: transfersTable.acceptedByClerkId,
    createdAt: transfersTable.createdAt,
    acceptedAt: transfersTable.acceptedAt,
  }).from(transfersTable)
...
router.post("/transfers", requireAuth(), async (req: Request, res: Response) => {
  ...
  const { fromLocationId, toLocationId, itemId, quantity, note } = req.body as {
    fromLocationId?: number; toLocationId?: number; itemId?: number; quantity?: number; note?: string;
  };
  ...
  if (!fromId || !toId || !item || !Number.isFinite(qty) || qty <= 0) {
    res.status(400).json({ error: "Укажите склад отправителя, склад получателя, товар и положительное количество" }); return;
  }
  ...
  try {
    const transfer = await db.transaction(async (tx) => {
      const stockRows = await tx.select().from(warehouseStockTable).where(and(eq(warehouseStockTable.itemId, item), eq(warehouseStockTable.locationId, fromId)));
      ...
      if (stock) {
        await tx.update(warehouseStockTable).set({ currentStock: sql`CAST(${warehouseStockTable.currentStock} AS DECIMAL) - ${qty}` }).where(eq(warehouseStockTable.id, stock.id));
      }
      const [created] = await tx.insert(transfersTable).values({
        fromLocationId: fromId, toLocationId: toId, itemId: item, quantity: String(qty), status: "pending", note: note?.trim() || null, createdByClerkId: scope.userId,
      }).returning();
      return created;
    });
    ...
    res.status(400).json({ error: error instanceof Error ? error.message : "Не удалось создать перемещение" });
  }
});
...
router.post("/transfers/:id/accept", requireAuth(), async (req: Request, res: Response) => {
  ...
  try {
    const transfer = await db.transaction(async (tx) => {
      ...
      if (stock) {
        await tx.update(warehouseStockTable).set({ currentStock: sql`CAST(${warehouseStockTable.currentStock} AS DECIMAL) + ${row.quantity}` }).where(eq(warehouseStockTable.id, stock.id));
      } else {
        await tx.insert(warehouseStockTable).values({ itemId: row.itemId, locationId: row.toLocationId, currentStock: row.quantity });
      }
    ...
    const message = error instanceof Error ? error.message : "Не удалось принять перемещение";
    res.status(message === "Перемещение не найдено" ? 404 : 400).json({ error: message });
  }
});
...
router.post("/transfers/:id/reject", requireAuth(), async (req: Request, res: Response) => {
  ...
  try {
    const transfer = await db.transaction(async (tx) => {
      ...
      if (sourceStock) {
        await tx.update(warehouseStockTable).set({ currentStock: sql`CAST(${warehouseStockTable.currentStock} AS DECIMAL) + ${row.quantity}` }).where(eq(warehouseStockTable.id, sourceStock.id));
      } else {
        await tx.insert(warehouseStockTable).values({ itemId: row.itemId, locationId: row.fromLocationId, currentStock: row.quantity });
      }
    ...
    const message = error instanceof Error ? error.message : "Не удалось отклонить перемещение";
    res.status(message === "Перемещение не найдено" ? 404 : 400).json({ error: message });
  }
});
